...
$("#prop").on('input change', function() {
	$(this).val($.trim($(this).val()).replace(/\s+/g,'')).removeClass("incorrect");
});
...
function generer() {
	...
	if (typNombre=="Entier") {
		nbrA = Math.floor(Math.random()*nbDif)+nbMin;
		nbrB = Math.floor(Math.random()*nbDif)+nbMin;
	}
	else if (typNombre=="Decimal") {
		if (chkForcDecimal==true) {
			nbrA = ((Math.random()*nbDif)+nbMin).toFixed(chApVirg);
			nbrB = ((Math.random()*nbDif)+nbMin).toFixed(chApVirg);
		}
		else {
			nbrA = Number(((Math.random()*nbDif)+nbMin).toFixed(chApVirg));
			nbrB = Number(((Math.random()*nbDif)+nbMin).toFixed(chApVirg));
		}
	}

	affNbrA = nbrA.toString();
	affNbrB = nbrB.toString();
	$("#nbrA").html(affNbrA.replace('.',','));
	$("#nbrB").html(affNbrB.replace('.',','));
}

$("#signInf").click(function() {
	$("#prop").removeClass("incorrect").val("<");
});

$("#signEgl").click(function() {
	$("#prop").removeClass("incorrect").val("=");
});

$("#signSup").click(function() {
	$("#prop").removeClass("incorrect").val(">");
});
...
function verifier() {
	...
	if ($("#prop").val()!="") {
		if (Number(nbrA)<Number(nbrB)) {
			soluce = "<";
		}
		else if (Number(nbrA)==Number(nbrB)) {
			soluce = "=";
		}
		else if (Number(nbrA)>Number(nbrB)) {
			soluce = ">";
	...
	if (nbSucces==1) {
		...
		totSucces ++;
		$("#affSucces").show();
		$("#succes").text("Résultat: "+totSucces+" / "+nbVerif);
		affSucces();
		enterSuivant=true;
