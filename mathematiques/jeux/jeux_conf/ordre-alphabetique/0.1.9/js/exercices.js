...
$("#btSuivant").click(function()
{
	...
	enterSuivant = false;
	numListMots ++;
	if (numListMots>=nbListMots) {
		alert("Cette série de mots est terminée. Tu peux la recommencer ou en choisir une aute.")
		numListMots = 0;
	}
	generer();
});
...
function distAlea(n){
	tblAlea=[];
	for (i=0;i<n;i++) {
		alea=Math.floor(Math.random()*n);
		if(tblAlea[alea]==undefined){tblAlea[alea]=i}else{i--}
	}
}
...
function generer() {
	...
	//
	//idxSelect = $("#lstSelect").prop('selectedIndex');
	//

	$("#consigne").html("Place les mots dans l'ordre alphabétique.");

	nbListMots = tbGlobal[idxSelect][1].length;
	tOrdre = tbGlobal[idxSelect][1][numListMots];

	tOrdre = tOrdre.sort(function(a, b){return a.localeCompare(b)});

	nbItems = tOrdre.length;
	distAlea(nbItems);

	$("#depoCompo").html("");

	for (var i=0; i<tOrdre.length; i++) {
		$("#depoCompo").append('<div id="box'+tblAlea[i]+'" class="box draggable" data-txt="'+tOrdre[tblAlea[i]]+'">'+tOrdre[tblAlea[i]]+'</div>');
	}
	$(".box").css({"border-color":colBord, "font-size":"1."+tailleFont+"em"});
}
...
function verifier() {
	...
	for (var i=0; i<nbItems; i++) {
		if (tOrdre[i]==tSort[i]) {
			$("#box"+i).removeClass("incorrect").addClass("correct");
			nbSucces += 1;
		}
		else {
			$("#box"+i).removeClass("correct").addClass("incorrect");			
		}
	}
