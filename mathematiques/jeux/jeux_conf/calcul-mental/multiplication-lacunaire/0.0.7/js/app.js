$(document).ready(function()
{
	...
	$("#rd1erT").click(function()
	{
		typInconnu = $(this).val();
		generer();
	});
	
	$("#rd2emT").click(function()
	{
		typInconnu = $(this).val();
		generer();
	});
	...
	if ($('#hdBox').length && $('#pseudoHd').html()!='')
	{
		...
		else
		{
			sessUser = false;
			sessProf = true;
			pseudoL = "calcul-mental-multiplication-lacunaire-"+$('#pseudoHd').html();
	...
	else
	{
		...
		pseudoL = "calcul-mental-multiplication-lacunaire-"+pseudo;
		lireStorageApparence();
		lireStorageProfil();
		verifStor();
	}
...
function lireOptions() {
	...
	$("#rd"+typInconnu).prop("checked", true);
