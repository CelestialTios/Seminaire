...
function verifPassDix(e) {
	...
	if (nbMax2C==0 && nbMax2D==0) {
		if (nbMax2U<=1) {
			nbMax2U = 2;
			$("#nbMax2U").val(2);
			alert("Le 2ème terme doit être supérieur à 1.")
		}
	}
...
function aleaTerms() {
	...
	n1 = Number(""+t1C+""+t1D+""+t1U);
	n2 = Number(""+t2C+""+t2D+""+t2U);
...
function generer() {
	...
	do {
		aleaTerms();
	} while (n2==0 || Number.isInteger(n1/n2) || n1<=n2);

	$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul">'+n1+' : '+n2+' = </div> <input type="tel" id="prop" value="" maxlength="3"><div id="soluc">'+result+'</div> <span class="txtRst">et il reste</span> <input type="tel" id="propR" val="" maxlength="1"></div>');
	...
	$("#propR").on('input change', function(){
		$(this).removeClass("incorrect");
		dejaVerif = false;
		$(this).val($.trim($(this).val()).replace(/\s+/g,''));
	});
...
function verifier() {
	...
	if ($("#propR").val()!="") {
		...
		else if ($("#propR").val()==reste) {
			$('#propR').removeClass("incorrect").addClass("correct");
			nbSucces += 1;
		}
		else {
			$('#propR').removeClass("correct").addClass("incorrect");
		}
	}
	else {
		alert("Un champ de réponse est vide.");
