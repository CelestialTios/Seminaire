...
dataV = "min";
dataAtt = "min";
window['nbMin1M'] = window['nbMin1C'] = window['nbMin1D'] = window['nbMin1U'] = 0;
window['nbMax1M'] = window['nbMax1C'] = window['nbMax1D'] = window['nbMax1U'] = 9;
...
function verifPassDix() {
	...
	for (var i=0; i<tbTerms.length; i++) {
		...
		if ($("#nbMin1"+term).is(":focus")==false) {
			if ($("#nbMin1"+term).val()=="") {
				/*window['nbMin1'+term] = 0;
				$("#nbMin1"+term).val(0);*/
				//window['nbMin1'+term] = window['nbMin1'+term];
		...
		if (dataAtt=="min") {
			if (nbAttMin>nbAttMax) {
				nbAttMax = nbAttMin;
				$("#nbAttMax").val(nbAttMax);
			}
		}
		else if (dataAtt=="max") {
			if (nbAttMax<nbAttMin) {
				nbAttMin = nbAttMax;
...
function generer() {
	...
	nVerif = Number(""+window['nbMin1M']+""+window['nbMin1C']+""+window['nbMin1D']+""+window['nbMin1U']);
	if (nVerif>nbAttMax) {
		$('.alert').show().html('').html('Attention: le 1er terme est plus grand que le nombre à atteindre.');
	}
	else {
		$('.alert').hide().html('');
		do {
			...
			nbAtt = Math.floor(Math.random() * (nbAttMax + 1 - nbAttMin) + nbAttMin);
	...
	$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul">'+n1+' + <div id="soluc">'+soluc+'</div> <input type="tel" id="prop" value="" size="6" maxlength="6"> = </div> <div id="result">'+nbAtt+'</div></div>');
