...
function verifPassDix() {
	...
	if (nbMax2C==0 && nbMax2D==0) {
		if (nbMax2U==0) {
			nbMax2U = 1;
			$("#nbMax2U").val(1);
			alert("Le 2ème terme doit être différent de 0.")
		}
	}
...
function generer() {
	...
	if (typInconnu=="2emT") {
		$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul">'+total+' : <input type="tel" id="prop" value="" maxlength="3"> = '+n1+'</div><div id="soluc">'+n2+'</div> </div>');
	}
	else if (typInconnu=="1erT") {
		$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul"><input type="tel" id="prop" value="" maxlength="3"> : '+n2+' = '+n1+'</div><div id="soluc">'+n1+'</div> </div>');
	}
