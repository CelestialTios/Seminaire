...
function verifPassDix() {
	...
	for (var i=0; i<tbTerms.length; i++) {
		...
		if (dataV=="min") {
			...
			if (window['nbMin2'+term]>window['nbMax1'+term]) {
				window['nbMax1'+term] = window['nbMin2'+term];
				$("#nbMax1"+term).val(window['nbMax1'+term]);
			}
		}
		else if (dataV=="max") {
			...
			if (window['nbMax1'+term]<window['nbMin2'+term]) {
				window['nbMin2'+term] = window['nbMax1'+term];
				$("#nbMin2"+term).val(window['nbMin2'+term]);
			}
		}

		/* Relation checkInput Passage 10 */ 				

		if (window['nbMin1'+term]==window['nbMax2'+term]) {
			window['chkPass'+term] = false;
			$("#chkPass"+term).prop({"disabled":true, "checked":false});
			$("#chkJust"+term).prop({"disabled":false});
		}
		else if (window['nbMin1'+term]>window['nbMax2'+term]) {
			window['chkPass'+term] = false;
			$("#chkPass"+term).prop({"disabled":true, "checked":false});
			...
			$("#chkPass"+term).prop({"disabled":false});
			$("#chkJust"+term).prop({"disabled":false});
		}

		/* Relation directe checkInput */

		if (window['chkPass'+term]==false) {
			/* Verif possibilité calcul */
			if (window['nbMax1'+term]==0) {
				// on ne retire pas 1 à nbMin2, et on traite ['nbMax1'+term]==0 dans aleaTerms()
			}
			else if (window['nbMin2'+term]==9) {
				window['nbMin2'+term] = 8;
				$("#nbMin2"+term).val(8);
			}
			else if (window['nbMax1'+term]<=window['nbMin2'+term]) {
				window['nbMin2'+term] = window['nbMax1'+term] -1;
				$("#nbMin2"+term).val(window['nbMax1'+term] -1);
			}
		}
		else if (window['chkPass'+term]==true) {
			window['just'+term] = false;
			$("#chkJust"+term).prop({"checked":false});
		}
		else if (window['chkJust'+term]==true) {
			window['chkPass'+term] = false;
			$("#chkPass"+term).prop({"checked":false});
		}				
	}
...
function aleaTerms() {
	...
	for (var i=0; i<tbTerms.length; i++) {
		...
		if (window['chkJust'+term]==true) {
			...
			} while (window['t1'+term]!=window['t2'+term]);
		}
		else if (window['chkPass'+term]==true) {
			do {
				window['t1'+term] = Math.floor(Math.random()*(window['nbMax1'+term]-window['nbMin1'+term]+1))+window['nbMin1'+term];
		...
		else if (window['chkPass'+term]==false) {
			if (window['nbMax1'+term]!=0) {
				do {
					window['t1'+term] = Math.floor(Math.random()*(window['nbMax1'+term]-window['nbMin1'+term]+1))+window['nbMin1'+term];
					window['t2'+term] = Math.floor(Math.random()*(window['nbMax2'+term]-window['nbMin2'+term]+1))+window['nbMin2'+term];
				...
				window['t1'+term] = window['t2'+term] = 0;
			}
		}
	}
...
function generer() {
	...
	if (typInconnu=="2emT") {
		$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul">'+n1+' - <input type="tel" id="prop" value="" maxlength="4"> = '+result+'</div><div id="soluc">'+n2+'</div></div>');
	}
	else if (typInconnu=="1erT") {
		$("#depoCompo").html("").append('<div id="calculBox"><div id="calcul"><input type="tel" id="prop" value="" maxlength="4"> - '+n2+' = '+result+'</div><div id="soluc">'+n1+'</div></div>');
	}
